...
TimetableApp.prototype.removeItemFromCell = function(cell, type = null, studentId = null) {
        ...
        // 如果有效版本来自更早的周（传播而来），不修改原始版本
        // 而是创建当前周的截断版本，仅影响当前周及以后
        const isPropagated = version.weekStart !== weekStartStr;

        if (isPropagated) {
            // 基于传播版本计算当前周的截断内容
            let newSubject = version.subject;
            let newStudents = (version.student || []).slice();
            ...
            // 直接在当前周创建截断版本
            if (!this.timetable[key] || !Array.isArray(this.timetable[key])) {
                this.timetable[key] = [];
            }
            // 移除当前周的已有版本（如果有）
            this.timetable[key] = this.timetable[key].filter(v => v.weekStart !== weekStartStr);
            // 插入截断版本（空内容用 _cutoff 标记防止被清理）
            this.timetable[key].push({
                weekStart: weekStartStr,
                subject: newSubject || null,
                student: newStudents || [],
                _cutoff: (!newSubject && (!newStudents || newStudents.length === 0)) || undefined
            });
            this.timetable[key].sort((a, b) => a.weekStart.localeCompare(b.weekStart));
        } else {
            // 版本就在当前周，直接修改
            if (type === 'subject') {
                version.subject = null;
            } else if (type === 'student' && studentId) {
                version.student = (version.student || []).filter(id => id !== studentId);
            } else if (type === 'student' && !studentId) {
                version.student = [];
            } else if (type === undefined || type === null) {
                // 双击/Delete键：清除整个课位
            ...
            // 如果修改后版本内容为空，检查是否有更早的版本会传播过来
            const modifiedSubject = version.subject;
            const modifiedStudents = version.student || [];
            const isEmpty = !modifiedSubject && modifiedStudents.length === 0;

            if (isEmpty) {
                const allVersions = this.timetable[key] || [];
                const hasPrevious = allVersions.some(v =>
                    v.weekStart < weekStartStr && (v.subject || (v.student && v.student.length > 0))
                );
                if (hasPrevious) {
                    // 转为截断版本，阻止更早的版本向前传播
                    version._cutoff = true;
                } else {
                    // 没有更早的版本，正常清理
                    this.setCellVersion(key, weekStartStr, null, []);
                }
            } else {
                // 用修改后的版本更新
                this.setCellVersion(key, weekStartStr, version.subject, version.student);
            }
        }
...
TimetableApp.prototype.moveSubject = function(sourceKey, targetKey) {
        ...
        // 迁移考勤记录
        if (this.attendance[sourceKey]) {
            if (!this.attendance[targetKey]) {
                this.attendance[targetKey] = {};
            }
            for (const date in this.attendance[sourceKey]) {
                this.attendance[targetKey][date] = this.attendance[sourceKey][date];
            }
            delete this.attendance[sourceKey];
        }
        if (this.studentAttendance[sourceKey]) {
            if (!this.studentAttendance[targetKey]) {
                this.studentAttendance[targetKey] = {};
            }
            for (const date in this.studentAttendance[sourceKey]) {
                this.studentAttendance[targetKey][date] = this.studentAttendance[sourceKey][date];
            }
            delete this.studentAttendance[sourceKey];
        }
        if (this.actualMinutes[sourceKey]) {
            this.actualMinutes[targetKey] = this.actualMinutes[sourceKey];
            delete this.actualMinutes[sourceKey];
        }
